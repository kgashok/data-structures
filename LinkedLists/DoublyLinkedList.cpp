...
void backwardTraversal(struct Node *tail)
{
    ...
    { // Loop continues until it reaches to start node
        cout << tail->info << " -> ";
        tail = tail->prev;
    }
...
struct Node *insertEndHead(struct Node *head)
{
    ...
    while (head->next != NULL)
    {                      // loops run it reaches the Tail Node
        head = head->next; //
    }
...
int main()
{
    ...
    while (choice != 8)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            tail = insertEndHead(head);
            break;
        case 3:
            tail = insertEndTail(tail);
            break;
        case 4:
            insertAfter(head);
            break;
